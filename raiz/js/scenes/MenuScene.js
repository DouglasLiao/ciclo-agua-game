...
export default class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.titleText = this.add
      .text(centerX, centerY - 60, loadingLabel, { fontSize: '42px', color: '#ffffff' })
      .setOrigin(0.5)
    this.startText = this.add
      .text(centerX, centerY + 10, startBtnLabel, { fontSize: '32px', color: '#555555' })
      .setOrigin(0.5)
    ...
    const attemptStart = async () => {
      ...
      this.startText.text = getPath(
        ...
        'ui.mensagens.iniciando',
        'Iniciando...'
      )
      try {
        if (!this.gameDataPromise)
          this.gameDataPromise = loadGameData(this.currentDataset).catch((err) => {
            throw err
          })
