...
export default class DragPhaseScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.targets = targetNames.map((name, i) => {
      ...
      const box = this.add
        .rectangle(x, yTargets, 160, 80, 0x123347, 0.6)
        .setStrokeStyle(2, 0x4ec2f0)
      this.add
        .text(x, yTargets - 10, name, {
          fontSize: '16px',
          color: '#ffffff',
          align: 'center',
          wordWrap: { width: 150 }
    ...
    this.scoreText = this.add.text(20, 20, `${scoreLabel}: 0/${blockLabels.length}`, {
      fontSize: '18px',
      color: '#ffffff'
    })
    const yBlocks = 420
    this.blocks = blockLabels.map((label, i) => {
      const x = startX + i * gapX
      const rect = this.add
        .rectangle(x, yBlocks, 150, 60, 0x345b7d, 0.8)
        .setStrokeStyle(2, 0xffffff)
      this.add
        .text(x, yBlocks, label, {
          fontSize: '16px',
          color: '#ffffff',
          align: 'center',
          wordWrap: { width: 140 }
